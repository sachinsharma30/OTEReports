...
def format_number(value, is_percent=False):
    ...
    return formatted_value + "%" if is_percent else formatted_value
...
for player_id in player_ids:
    ...
    # Extract per-game stats and format values
    row_data = [
        player_name,
        format_number(season_stats.get("games", 0)),  # GP
        format_number(season_stats.get("minutes_per_game", 0.0)),  # MIN per game
        format_number(season_stats.get("points_per_game", 0)),  # PTS per game
        format_number(season_stats.get("rebounds_offensive_per_game", 0)),  # ORB per game
        format_number(season_stats.get("rebounds_defensive_per_game", 0)),  # DRB per game
        format_number(season_stats.get("rebounds_per_game", 0)),  # TRB per game
        format_number(season_stats.get("assists_per_game", 0)),  # AST per game
        format_number(season_stats.get("turnovers_per_game", 0)),  # TO per game
        format_number(season_stats.get("steals_per_game", 0)),  # STL per game
        format_number(season_stats.get("blocks_per_game", 0)),  # BLK per game
        format_number(season_stats.get("fouls_personal_per_game", 0)),  # PF per game
        format_number(season_stats.get("fouls_drawn_per_game", 0)),  # FD per game
        format_number(season_stats.get("points_two_made_per_game", 0)),  # 2PM per game
        format_number(season_stats.get("points_two_attempted_per_game", 0)),  # 2PA per game
        format_number(season_stats.get("points_two_percentage", 0.0), is_percent=True),  # 2P%
        format_number(season_stats.get("points_three_made_per_game", 0)),  # 3PM per game
        format_number(season_stats.get("points_three_attempted_per_game", 0)),  # 3PA per game
        format_number(season_stats.get("points_three_percentage", 0.0), is_percent=True),  # 3P%
        format_number(season_stats.get("field_goals_made_per_game", 0)),  # FGM per game
        format_number(season_stats.get("field_goals_attempted_per_game", 0)),  # FGA per game
        format_number(season_stats.get("field_goals_percentage", 0.0), is_percent=True),  # FG%
        format_number(season_stats.get("free_throws_made_per_game", 0)),  # FTM per game
        format_number(season_stats.get("free_throws_attempted_per_game", 0)),  # FTA per game
        format_number(season_stats.get("free_throws_percentage", 0.0), is_percent=True),  # FT%
        format_number(season_stats.get("plus_minus_per_game", 0)),  # +/- per game
    ]
...
print(f"✅ Player averages saved to `{csv_filename}` with sorted player names.")
